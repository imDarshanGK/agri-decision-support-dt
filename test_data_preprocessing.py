...
def test_load_data_missing_file_raises(project_dir, monkeypatch):
    # Remove both the primary path and prevent fallback
    os.remove("data/raw/Crop_recommendation.csv")
    # Mock DATASET_ALT_PATH to return a non-existent path
    from pathlib import Path
    monkeypatch.setattr(config, "DATASET_ALT_PATH", Path("/nonexistent/Crop_recommendation.csv"))
    
    with pytest.raises(Exception):  # Should raise DataNotFoundError
        dp.load_data()
...
def test_split_data_shapes(synthetic_df):
    ...
    assert set(X_train.columns) == set(config.FEATURE_COLS)
